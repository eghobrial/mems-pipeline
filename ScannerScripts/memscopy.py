	...
	if PList:
		...
		for etime,fname,pid,sdate,stime,rds,fsize in PList:
			...
			if (tdiff < (60*60*6)): #to include files that span up to 6 hours
				...
				etimelast = etime #used to get last time
...
def readtempfile(stime,etime):
	file = open("/usr/g/service/log/paramdata/sriTemp.xml", "r")
	wfile = open("/tmp/mems"+timestamp+"/temp.csv", "w")
	lines = file.readlines()
	file.close()

	#stime = time.mktime(time.strptime("02-26-2013 10:00:00", "%m-%d-%Y %H:%M:%S"))
	#etime = time.mktime(time.strptime("02-26-2013 10:30:00", "%m-%d-%Y %H:%M:%S"))
	stime = stime - (60*15)
	etime = etime + (60*15)
	for index in range(len(lines)):
		...
		if match:
			rawtemp = lines[index+1].lstrip()
			#print rawtemp
			a = re.split(',|"|>|<|\.',rawtemp)
			ftime = time.mktime(time.strptime(a[4], "%m-%d-%Y %H:%M:%S"))
			if ((stime < ftime ) & (etime > ftime )):
				wfile.write(a[4] + "," + a[7] + "," + a[8] + "," + a[9] + "," + a[10] + "\n")
